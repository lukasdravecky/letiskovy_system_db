...
CREATE TABLE krajina (
                         id_krajina  INT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
                         nazov       TEXT        NOT NULL CHECK (TRIM(nazov) <> ''),--check ci nazov nie je prazdny retazec
                         UNIQUE (nazov)
);

CREATE TABLE mesto (
                       id_mesto    INT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
...
CREATE TABLE letisko (
                         id_letisko  INT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
                         nazov       TEXT        NOT NULL CHECK (TRIM(nazov) <> ''),
                         id_mesto    INT         NOT NULL,
                         iata_kod    VARCHAR(3)  NOT NULL CHECK (iata_kod ~ '^[A-Z]{3}$'),--check ci kod je zapisany pismenami velkej abecedy A-Z a ma dlzku prave 3
                         icao_kod    VARCHAR(4)  NOT NULL CHECK (icao_kod ~ '^[A-Z]{4}$'),-- ==||== a ma dlzku prave 4
                         UNIQUE (iata_kod),
                         UNIQUE (icao_kod),
...
CREATE TABLE terminal (
                          id_terminal INT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
                          nazov       TEXT        NOT NULL CHECK (TRIM(nazov) <> ''),
...
CREATE TABLE stav_brany (
                            id_stav_brany   INT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
                            stav            TEXT    NOT NULL CHECK (TRIM(stav) <> ''),
                            popis           TEXT
);

CREATE TABLE brana (
                       id_brana        INT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
...
CREATE TABLE letecka_spolocnost (
                                    id_spolocnost   INT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
                                    nazov           TEXT    NOT NULL CHECK (TRIM(nazov) <> ''),
...
CREATE TABLE lietadlo (
                          id_lietadlo     INT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
                          model           TEXT    NOT NULL CHECK (TRIM(model) <> ''),
                          kapacita        INT     NOT NULL CHECK (kapacita > 0),
                          rok_vyroby      INT     CHECK (rok_vyroby >= 1900 AND rok_vyroby <= EXTRACT(YEAR FROM CURRENT_TIMESTAMP)),--check ci rok_vyroby je novsi ako 1900 a starsi ako CURRENT_TIMESTAMP
...
CREATE TABLE stav_letu (
                           id_stav_letu    INT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
                           stav            TEXT    NOT NULL CHECK (TRIM(stav) <> ''),
                           popis           TEXT
);

CREATE TABLE let (
                     id_let                  INT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
                     cislo_letu              TEXT        NOT NULL CHECK (TRIM(cislo_letu) <> ''),
                     cas_odletu              TIMESTAMP WITH TIME ZONE NOT NULL,
                     cas_priletu             TIMESTAMP WITH TIME ZONE NOT NULL CHECK (cas_priletu > cas_odletu),--check ci je cas priletu skorsi ako odletu (timezone riesi problem aj s priletom v inom casovom pasme)
...
CREATE TABLE priradenie_brany (
                                  id_priradenie   INT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
                                  id_let          INT         NOT NULL,
                                  id_brana        INT         NOT NULL,
                                  cas_od          TIMESTAMP WITH TIME ZONE NOT NULL,
                                  cas_do          TIMESTAMP WITH TIME ZONE CHECK (cas_do IS NULL OR cas_do > cas_od),--cas_do moze byt null ALEBO musi byt neskorsi ako cas_od
...
CREATE TABLE cestujuci (
                           id_cestujuci        INT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
...
CREATE TABLE letenka (
                         id_letenka          INT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
                         cislo_sedadla       TEXT        NOT NULL CHECK (TRIM(cislo_sedadla) <> ''),
                         je_business         BOOLEAN     NOT NULL,
                         cena                NUMERIC     NOT NULL CHECK (cena > 0),--nezaporna a nenulova cena
...
CREATE TABLE stav_batoziny (
                               id_stav_batoziny    INT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
                               stav                TEXT    NOT NULL CHECK (TRIM(stav) <> ''),
                               popis               TEXT
);

CREATE TABLE batozina (
                          id_batozina         INT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
                          hmotnost            NUMERIC   NOT NULL CHECK (hmotnost > 0),--nezaporna a nenulova hmotnost
